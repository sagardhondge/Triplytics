...
export default function Navbar() {
  ...
  const navLinks = [
    ...
    { name: "Vehicle", path: "/vehicle", icon: <Car size={20} /> },
    { name: "Cost Details", path: "/cost-details", icon: <Wallet size={20} /> },
  ...
  return (
    <nav className="bg-blue-600 text-white shadow-md fixed w-full top-0 left-0 z-50">
      ...
      {/* Mobile Menu */}
      {isOpen && (
        <div className="md:hidden bg-blue-700 py-4 space-y-4 px-6">
          {navLinks.map((link) => (
            <Link
              key={link.name}
              to={link.path}
